...
export class AuthController {
  ...
  async login(@Body() loginDto: LoginDto, @Request() req: any) {
    ...
    if (user.twoFactorEnabled && !loginDto.twoFactorCode) {
      // Return a short-lived signed MFA token to prevent auth bypass
      const mfaToken = this.authService.generateMfaToken(user, hotelId);
      return {
        requires_2fa: true,
        mfaToken,
      };
  ...
  async verify2fa(@Body() dto: Verify2faDto, @Request() req: any) {
    ...
    // We must verify the MFA token which was signed after password verification
    const mfaPayload = this.authService.verifyMfaToken(dto.mfaToken);

    const user = await this.authService.findUserById(mfaPayload.sub);
    ...
    return this.authService.login(user, mfaPayload.hotelId, metadata);
  }
