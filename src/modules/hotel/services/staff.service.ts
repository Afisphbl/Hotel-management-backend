...
export class StaffService {
  ...
  async findAll(hotelId: string, options: { page?: number; limit?: number; role?: StaffRole; status?: StaffStatus; department?: string }) {
    ...
    if (options.role) { params.push(options.role); conditions.push(`role = $${params.length}`); }
    if (options.status) { params.push(options.status); conditions.push(`status = $${params.length}`); }
    if (options.department) { params.push(options.department); conditions.push(`department = $${params.length}`); }

    const where = conditions.join(' AND ');
    const [rows, countResult] = await Promise.all([
      this.dataSource.query(`SELECT * FROM "${s}"."staff" WHERE ${where} ORDER BY "firstName" ASC, "lastName" ASC LIMIT ${limit} OFFSET ${offset}`, params),
      this.dataSource.query(`SELECT COUNT(*)::int AS count FROM "${s}"."staff" WHERE ${where}`, params),
    ]);
  ...
  async findById(id: string, hotelId: string) {
    const s = await this.getSchema(hotelId);
    const rows = await this.dataSource.query(`SELECT * FROM "${s}"."staff" WHERE id = $1 AND "deletedAt" IS NULL`, [id]);
  ...
  async create(data: any, hotelId: string) {
    ...
    const rows = await this.dataSource.query(
      ...
      [data.userId ?? '', data.firstName, data.lastName, data.email, data.phone ?? null,
       data.role, data.employmentType ?? 'full_time', data.status ?? 'active',
       data.hourlyRate ?? null, data.department ?? null, data.joinedAt ?? null],
    );
    return rows[0];
  }
  ...
  async update(id: string, data: any, hotelId: string) {
    const s = await this.getSchema(hotelId);
    const allowed = ['firstName', 'lastName', 'email', 'phone', 'role', 'employmentType', 'status', 'hourlyRate', 'department'];
    const fields: string[] = [];
    const params: any[] = [];
  ...
  async remove(id: string, hotelId: string) {
    const s = await this.getSchema(hotelId);
    await this.dataSource.query(`UPDATE "${s}"."staff" SET "deletedAt" = NOW() WHERE id = $1 AND "deletedAt" IS NULL`, [id]);
  }
}
