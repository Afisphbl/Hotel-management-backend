...
import { runWithTenantSchema } from '../src/common/tenant/tenant-context';
...
async function bootstrap() {
  ...
  try {
    ...
    for (const hc of HOTELS) {
      ...
      await provisionTenantSchema(dataSource, hotel.schemaName);
      ...
      await runWithTenantSchema(hotel.schemaName, async () => {
...
async function provisionTenantSchema(dataSource: DataSource, schemaName: string): Promise<void> {
  const s = schemaName;
  await dataSource.query(`
